...
| **19** | Maestro RAG dev scaffolding (Vault-only semantics) | RAG training endpoints; embeddings index scoped to Vault docs only; Maestro context API surfaces; feature flags keeping RAG dev-only and off for GA tenants. | ✅ Done (dev-only; RAG not exposed to GA users) |
