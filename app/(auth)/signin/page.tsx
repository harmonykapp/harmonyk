...
export default function SignInPage() {
  ...
  async function sendMagicLink(e: React.FormEvent) {
    ...
    else { setStatus("sent"); setMessage("Magic link sent. Check your email."); }
  }
  ...
  return (
    <div style={{maxWidth:420, margin:"80px auto", fontFamily:"system-ui, sans-serif"}}>
      ...
      <div style={{opacity:.6, fontSize:12, margin:"8px 0"}}>or</div>
      ...
      <form onSubmit={sendMagicLink} style={{display:"grid", gap:12}}>
        ...
        <button type="submit" disabled={status==="sending" || email.trim()===""}
          style={{padding:"10px 12px", border:"1px solid #6B4FEF",
                  background:"#6B4FEF", color:"#fff", borderRadius:8, cursor:"pointer",
      ...
      {message && (
        <p style={{marginTop:12, color: status==="error" ? "#B00020" : "#2f6f2f"}}>{message}</p>
      )}
    </div>
  );
}
