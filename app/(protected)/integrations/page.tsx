...
export default function IntegrationsPage() {
  ...
  const Card = ({
    ...
    <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 16 }}>
      ...
      <p style={{ opacity: 0.75, margin: "6px 0 12px" }}>{desc}</p>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, opacity: 0.6 }}>Status: {state}</span>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card
        title="Google Drive"
        desc="List Recents (RO) and save to an app folder."
        state={drive}
        onConnect={connectDrive}
        onReauth={reauthDrive}
        onDisconnect={disconnectDrive}
      />
      <Card
        title="Gmail"
        desc="Read metadata (subjects, senders) for Workbench filters."
        state={gmail}
        onConnect={connectGmail}
        onReauth={reauthGmail}
