...
export function BuilderClient({ templates, clauses, deckTemplates = [], initialDocId, initialTab }: BuilderClientProps) {
  ...
  const [trainingQueuedForDocId, setTrainingQueuedForDocId] = useState<string | null>(null);
  const [trainingError, setTrainingError] = useState<string | null>(null);

  const [previewingMonoContext, setPreviewingMonoContext] = useState(false);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const stub = [
        ...
        clauseText,
        "",
        "In production, contract text is generated by Maestro and the Contracts Builder, not this placeholder.",
      ].join("\n");
  ...
  const handleTrainWithMono = async () => {
    if (!monoTrainingEnabled) {
      toast({
        title: "Training disabled",
        description: "Maestro training is not enabled in this environment.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!savedDocumentId) {
      toast({
        title: "Save to Vault first",
        description: "Save the document to Vault before training Maestro on it.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (error || !data?.org_id) {
        const errorMessage =
          error?.message || "Could not determine organisation for this document.";
        console.error("[builder] Maestro training: failed to resolve org", { error, data });
      ...
      const response = await fetch("/api/mono/train", {
        ...
        body: JSON.stringify({
          orgId: data.org_id,
          vaultDocumentId: savedDocumentId,
        }),
      });

      const text = await response.text();
      let payload: { job?: unknown; error?: string } = {};
      try {
      ...
      if (!response.ok || payload.error) {
        ...
        handleApiError({
          ...
          errorMessage,
          toast,
          context: "builder-mono-train",
        });
        return;
      }

      // Success: API returns {job: {...}} on success
      setTrainingQueuedForDocId(savedDocumentId);
      toast({
  ...
  const handlePreviewMonoContextDev = async () => {
    if (!monoTrainingEnabled) {
      toast({
        title: "Maestro context disabled",
        description: "Maestro dev context preview is not enabled in this environment.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!savedDocumentId) {
      toast({
        title: "Save to Vault first",
        description: "Save the document to Vault before previewing Maestro context.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (error || !data?.org_id) {
        const errorMessage =
          error?.message || "Could not determine organisation for this document.";
        console.error("[builder] Maestro context: failed to resolve org", { error, data });
        handleApiError({
          status: 500,
          errorMessage,
          toast,
          context: "builder-mono-context",
        });
        return;
      }
      ...
      const requestBody = {
        ...
        query: effectiveTitle.slice(0, 200),
        maxItems: 5,
      };
      ...
      const text = await response.text();
      let payload: MonoContextResponse | { error?: string } = { docs: [] };

      try {
        payload = text
          ? (JSON.parse(text) as MonoContextResponse | { error?: string })
          : { docs: [] };
      } catch {
        payload = { error: text || "Failed to parse Maestro context response" };
      }
      ...
      const docs = (payload as MonoContextResponse).docs ?? [];

      try {
        console.groupCollapsed("[builder-mono-context] Preview Maestro context (dev)");
      ...
      toast({
        title: "Maestro context loaded",
        description: docs.length
          ? `Found ${docs.length} matching internal docs for this organisation.`
          : "No internal docs matched this query yet.",
      });
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error loading Maestro context";
      ...
      setPreviewingMonoContext(false);
    }
  };
              ...
              <div className="text-center max-w-md space-y-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mb-2">
                    Draft investor and operational contracts with AI. Start from a template or a prompt.
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Select a template from the sidebar to begin creating your document.
                    Maestro will help you draft, review, and refine the content.
                  </p>
                </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-auto p-6">
                <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  {monoTrainingEnabled && trainingQueuedForDocId === savedDocumentId && (
                    <div className="rounded-md border border-blue-500/40 bg-blue-500/5 px-3 py-2 text-xs text-blue-700 dark:text-blue-400">
                      Maestro training job queued for this document.
                    </div>
                  )}

                  {monoTrainingEnabled && trainingError && (
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            Describe what you need and Maestro will draft the document for you
                          </CardDescription>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <Sparkles className="h-4 w-4 text-mono" />
                          <span>Ask Maestro about building</span>
                        </div>
                      </div>
                    </CardHeader>
    ...
    const handleSaveDeckToVault = async () => {
      ...
      if (generatedDeckSections.length === 0) {
        toast({
          ...
          description: "Generate a draft with Maestro before saving to Vault.",
          variant: "destructive",
        });
        return;
      }
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="text-center max-w-md space-y-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mb-2">
                    Build fundraising decks and investor updates in minutes.
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Select a template from the sidebar to get started. Maestro will help you generate content for each section.
                  </p>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            This outline drives the slides Maestro will generate. You can rename, disable, or reorder sections.
                          </CardDescription>
                        </div>
                        <Button
                          variant="outline"
                ...
                {selectedDeckTemplate && deckOutlineSections.length > 0 && (
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Provide company information to help Maestro generate relevant content.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                    ...
                    <CardContent className="pt-6">
                      <div className="space-y-4">
                        <div className="text-sm text-muted-foreground">
                          Maestro will generate slide content from this outline and your company info in one pass. Make sure you have at least{" "}
                        ...
                        >
                          <Sparkles className="h-4 w-4 mr-2" />
                          {isGeneratingDeck ? "Generating..." : "Generate Deck with Maestro"}
                        </Button>
  ...
  const renderAccountsBuilder = () => {
    ...
    return (
      <div className="h-full flex">
                    ...
                    <div className="max-w-lg text-center space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-muted-foreground text-sm mb-2">
                          Pull out founder-friendly financial insights from your raw numbers.
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Upload a spreadsheet or connect a source, then run a pack to see insights.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        {/* Pack preview dialogs (post-GA enhancement; GA focuses on core pack runs) */}
