...
export default function StatusPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res: StatusResponse = await fetch("/api/integrations/status", {
        cache: "no-store",
      }).then((r) => r.json());
      setStatus(res.googleDrive ?? "unknown");
    } catch (err: unknown) {
    ...
    <section className="rounded-2xl border p-4">
      ...
      {!loading && !error && status === "needs_reauth" && (
        <div className="text-sm">
          ...
          <a href="/integrations" className="ml-1 underline">
            Open Integrations
          </a>{" "}
          to re-authenticate.
        </div>
      ...
      {!loading && !error && status === "connected" && (
        <div className="text-sm opacity-70">
          All good. Recent (RO) will populate once OAuth is wired.
        </div>
      )}
